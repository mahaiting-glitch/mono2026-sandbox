...
3、**Pinia store 用 setup 风格**——`defineStore('todo', () => {...})`、不写 options 形式。
   ...
   3e、**store action 有失败分支时返回 `T | null`、调用侧显式 null 判断**——「失败分支」指 action 内部已 catch 并希望调用侧做分支处理（若 throw 让上层 try/catch 接管则不适用本条）。action 返回类型必须是 `T | null`（不返 `boolean` / `void` / `T | undefined`）；调用侧先赋给具名变量，再用 `!== null` 显式判断，不用 `if (result)` 隐式 truthy 短路（`0` / `''` / `false` 等合法值会误判）。❌ action 签名：`async function createList(name: string): Promise<boolean>` → ✅ `async function createList(name: string): Promise<List | null>`。❌ 调用侧：`if (await store.createList(name))` → ✅ `const created = await store.createList(name); if (created !== null) { ... }`。（内部实现见 3f）

   3f、**action 内部必须 catch 所有异步异常归一为 `T | null` 返回**——3e（调用侧契约）与本条（action 内部实现）配套：action 内部的网络请求 / `JSON.parse` / 存储读写（localStorage / IndexedDB）等可能 throw 的操作必须包裹在 `try/catch` 中，`catch` 里至少 `console.error('[<store>.<action>] failed:', err)` 后 `return null`；禁止裸 `catch {}` 静默吞错；禁止让异步异常透传到组件层（Vue 全局 `app.config.errorHandler` 只是兜底，不是约定的错误边界）；若确需 throw（限于：编程错误如参数非法 / 断言失败，或顶层导航守卫致命错误；普通网络失败 / 4xx / 5xx 不属于），需在函数注释里显式标注 `@throws <ErrorType> <触发条件>`。❌ `async function fetchTodo(id: string): Promise<Todo | null> { return await api.get(id) /* api.get throws on network error / 4xx / 5xx */ }` （未 catch，异步异常 throw 到组件）→ ✅ `async function fetchTodo(id: string): Promise<Todo | null> { try { return await api.get(id) } catch (err) { console.error('[todoStore.fetchTodo] failed:', err); return null } }`。

   3g、**禁止对 `T | null | undefined` 值用 `!` 非空断言——必须走显式分支**——3e 要求 action 返回 `T | null`、调用侧必须分支处理；本条封堵「懒人写法」：直接用 `!` 把 `T | null | undefined` 断言成 `T`（包括链式 `!.` 取属性）等于绕过了 null/undefined 判断、让安全契约形同虚设，且在 null/undefined 时运行期 throw `TypeError`。同源的 nullable 值（`Array.find` 返回 `T | undefined`、`Map.get` 返回 `T | undefined`、可选 prop）同样适用。必须先赋给具名变量再显式判断，或早返（`if (result == null) return`）。❌ `const x = (await store.createList(name))!`（`!` 抹掉 null、null 时崩）；❌ `const title = (await store.fetchTodo(id))!.title`（链式 `!.`、null 时崩）；❌ `items.find(i => i.id === id)!.name`（find 返回 `T | undefined`、未匹配时崩） → ✅ `const x = await store.createList(name); if (x !== null) { ... }`（显式分支）或 `const x = await store.createList(name); if (x == null) return;`（早返、`== null` 同时排 null 和 undefined）。

   3h、**禁止 `?.` 与 `!` 在同一表达式串用**——`obj?.foo!` / `obj?.foo!.bar` 等写法自相矛盾：`?.` 表示「我知道左侧可能是 null/undefined」，`!` 紧接着却声明「结果一定非 null/undefined」；当 `obj` 为 null/undefined 时，`?.` 短路返回 `undefined`，后续 `!` 不能阻止运行期 `TypeError`（TypeScript 编译期仅静默、不保护运行期）。二选一：确定非 null 就用 `.`（显式前置判断或早返后），不确定就用 `?.` 并处理 `undefined` 分支。如已引入 `typescript-eslint`，可用 `@typescript-eslint/no-non-null-asserted-optional-chain: error` 在工具链层拦截。❌ `user?.profile!.name`（`user` 为 null 时 `?.` 返回 undefined，`!` 无法阻止访问 `.name` throw）；❌ `store.getItem(id)?.value!`（同上）；❌ `arr?.find(i => i.id === id)!.name`（`arr` 为 null 时同样 throw）→ ✅ `const profile = user?.profile; if (profile != null) { profile.name }` 或早返后直接用 `.`：`if (user == null) return; user.profile.name`（`?.` 短路不会触发）。
...
- ❌（3e）action 返回 `boolean` / `void` / `T | undefined` 表达失败（`if (result)` 对 `0` / `''` / `false` 等合法值误判）→ ✅ action 返回 `T | null`，调用侧 `const created = await store.createList(name); if (created !== null) { ... }`（显式 null 判断）
